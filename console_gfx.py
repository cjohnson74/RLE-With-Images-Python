    ...
    def display_image2(image_data, top, up_left, up_right, start, end, bottom, low_left, low_right):
        ...
        for y_index in range(0, height, 2):
            ...
            for x_index in range(width):
                ...
                color_index = ConsoleGfx.TRANS_DISPLAY if output_color == ConsoleGfx.CLEAR else output_color
                output_str += ConsoleGfx.fg_palette[color_index]
                output_color = image_data[data_index + width] if y_index + 1 < height else ConsoleGfx.CLEAR
                color_index = ConsoleGfx.TRANS_DISPLAY if output_color == ConsoleGfx.CLEAR else output_color
                output_str += ConsoleGfx.bg_palette[color_index]
                output_str += '▀'
                data_index += 1
            data_index += width
